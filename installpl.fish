...
if test -f "$dir/$name/manifest.toml"
    ...
    echo "	Error: manifest.toml not found in $dir/$name"
    exit 1
end

echo "- Building plugin `$name` from dir `$dir`..."
cd "$dir/$name"
cargo build -q --release > ./logs

if test -f "$dir/$name/target/release/lib$name.so"
    echo "	Build success"
else
    echo "	Error while building plugin"
    exit 1
end
...
echo "- Adding to config file..."
if grep -q $name $original_dir/nocast.toml
	echo "	Already added"
else
	echo "$name = \"$target/$name.toml\"" >> $original_dir/nocast.toml	
end
